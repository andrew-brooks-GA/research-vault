...
function findEntry(vaultPath, id) {
  for (const abs of walkEntries(vaultPath)) if (abs.replace(/\.md$/, '').endsWith(id)) return abs;
...
export async function run(args) {
  ...
  const r = applyVerification(vaultPath, { id: args.id, method: args.method, result: args.result, byId: args['by-id'], notes: args.notes, supersededBy: args['superseded-by'] });
