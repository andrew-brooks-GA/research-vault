...
---
# Verifying an entry
Follow `meta/prompt-templates/verify-entry.md`. Pick a method; for entries about a tool with a `meta/probe-manifests/` manifest, prefer `tool-probe` (follow `meta/prompt-templates/probe-tool.md`; same-version only — a version mismatch is version succession, not outdated). Re-reading an entry is NOT verification. `inferred-stable` only for volatility=stable + durable source. The `captured` seed `capture` writes at creation time is provenance, not verification (`verify` rejects it).
...
Offline: only `inferred-stable`, `human-spot-check`, and `tool-probe` (probes run locally) are valid.
