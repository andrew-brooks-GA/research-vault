...
3. Pick a method: `refetched-source`, `cross-referenced`, `existence-check` (weak), `human-spot-check`, or `inferred-stable` (only stable + durable book/paper/talk). (`captured` is the creation-time seed only; `verify` rejects it.)
